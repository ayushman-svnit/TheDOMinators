...
function MovingVehicle({ mode, routeHeight }) {
  ...
  useFrame((state, delta) => {
    if (vehicleRef.current) {
      ...
      if (mode === "aerotaxi" || mode === "drone") {
        vehicleRef.current.position.y += Math.sin(state.clock.elapsedTime * 3) * 0.2
      }
    }
  })
  ...
  const getVehicleGeometry = () => {
    switch (mode) {
      case "hyperloop":
        return <Cylinder args={[0.3, 0.3, 1.5]} rotation={[0, 0, Math.PI / 2]} />
      case "robotaxi":
  ...
  return (
    <group ref={vehicleRef}>
      {getVehicleGeometry()}
      <meshStandardMaterial color={getVehicleColor()} emissive={getVehicleColor()} emissiveIntensity={0.3} />
    </group>
  )
}
  ...
  const buildings = useMemo(() => {
    return Array.from({ length: 40 }, (_, i) => {
      ...
      return {
        ...
        color: Math.random() > 0.6 ? "#1e40af" : Math.random() > 0.3 ? "#1e293b" : "#0f172a",
  ...
  return (
    <group>
      {buildings.map((building, i) => (
        <group key={i}>
          <Box args={[1, 1, 1]} position={building.position} scale={building.scale}>
                ...
                <group key={floor}>
                  <Box
                    ...
                    position={[
                      ...
                      building.position[1] - building.scale[1] / 2 + (floor + 0.5) * 2,
    ...
    <group>
      ...
      {trees.map((tree, i) => (
        <group key={i} position={tree.position}>
          {/* Trunk */}
          <Cylinder args={[0.1, 0.15, tree.height]} position={[0, tree.height / 2, 0]}>
  ...
  return (
    <group>
      ...
      {(selectedMode === "aerotaxi" || selectedMode === "drone") && (
        <>
          {Array.from({ length: 5 }, (_, i) => (
            <Cylinder key={i} args={[0.2, 0.2, routeHeight]} position={[-8 + i * 4, routeHeight / 2, 0]}>
              <meshStandardMaterial color="#6b7280" />
            </Cylinder>
          ))}
        </>
      ...
      {selectedMode === "hyperloop" && (
        <Cylinder args={[0.8, 0.8, 20]} position={[0, routeHeight + 0.5, 0]} rotation={[0, 0, Math.PI / 2]}>
          <meshStandardMaterial color="#1e1b4b" transparent opacity={0.6} metalness={0.8} roughness={0.2} />
        </Cylinder>
      )}

      {/* Moving Vehicle */}
      {selectedMode && <MovingVehicle mode={selectedMode} routeHeight={routeHeight} />}

      {/* Route Markers */}
      <Text position={[-12, routeHeight + 2, 0]} fontSize={0.8} color="#ffffff" anchorX="center" anchorY="middle">
        START
      </Text>
...
function CityScene({ selectedMode }) {
  return (
    <group>
      ...
      {/* City Name */}
      <Text position={[0, 15, -20]} fontSize={2} color="#3b82f6" anchorX="center" anchorY="middle">
        Smart City 2035
      </Text>

      {/* Mode Label */}
      {selectedMode && (
        <Text position={[0, 12, -15]} fontSize={1.2} color="#ffffff" anchorX="center" anchorY="middle">
