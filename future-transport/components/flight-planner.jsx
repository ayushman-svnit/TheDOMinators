...
import { Plane, Zap, Battery, Cloud, MapPin, Clock, AlertTriangle, Route, Wind } from "lucide-react"
...
const skyLanes = [
  ...
  },
  {
    ...
    traffic: "High",
    weather: "Clear",
    cities: ["New Delhi", "Chandigarh", "Amritsar", "Jaipur", "Lucknow", "Agra"],
  },
  {
    id: "lane-4",
...
export function FlightPlanner() {
  ...
  const distance = sourceCity && destinationCity ? getDistance(sourceCity, destinationCity) : 0

  const generateFlightPlan = () => {
    if (!selectedMode || !sourceCity || !destinationCity) return

    const selectedModeData = flightModes.find((mode) => mode.id === selectedMode)
    ...
    const totalCost = Math.round(selectedModeData.costPerKm * calculatedDistance * 83) // Convert to INR
    const rangeValue = Number.parseInt(selectedModeData.range.replace("km", "")) // Extract number from "25km"
    const batteryUsage = Math.min(Math.round((calculatedDistance / rangeValue) * 100), 100)

    // Determine optimal sky lane
    const availableLanes = skyLanes.filter(
      (lane) => lane.cities.includes(sourceCity) && lane.cities.includes(destinationCity),
    )
    const optimalLane = availableLanes.length > 0 ? availableLanes[0] : skyLanes[0]
    ...
    const plan = {
      ...
      duration: flightTime > 60 ? `${Math.floor(flightTime / 60)}h ${flightTime % 60}m` : `${flightTime} minutes`,
      ...
      fuelEfficiency: Math.round((calculatedDistance / Number.parseInt(selectedModeData.batteryLife)) * 10) / 10,
    }
  ...
  const getAvailableSkyLanes = () => {
    ...
    return skyLanes.filter((lane) => lane.cities.includes(sourceCity) || lane.cities.includes(destinationCity))
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-black/20 border-blue-500/30">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-gray-400 text-sm mb-2 block">Departure City</label>
            ...
            <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              <div className="flex items-center gap-4 text-white">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm">Aerial Distance: {distance.toLocaleString()} km</p>
                </div>
                <Badge className="bg-green-500/20 text-green-400">Route Available</Badge>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {flightModes.map((mode) => {
          ...
          const isInRange = calculatedDistance === 0 || calculatedDistance <= rangeInKm
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <p className="text-white font-semibold">{mode.maxAltitude}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-white font-semibold">{mode.batteryLife}</p>
                  </div>
                  <div>
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-cyan-400 font-semibold">₹{Math.round(mode.costPerKm * 83)}</span>
                </div>
                {!isInRange && calculatedDistance > 0 && (
                  <Badge variant="secondary" className="bg-red-500/20 text-red-400 w-full justify-center">
                    Out of Range
                  </Badge>
                )}
                {isSelected && <Badge className="bg-green-500/20 text-green-400 w-full justify-center">Selected</Badge>}
              </CardContent>
            </Card>
          )
        })}
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-center">
            <Button
              ...
              disabled={!selectedMode || !sourceCity || !destinationCity || distance === 0}
            >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {availableSkyLanes.map((lane) => (
              <div key={lane.id} className="p-4 rounded-lg bg-gray-800/50 border border-gray-700/50">
                  ...
                  <div className="mt-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {lane.cities.length > 3 && (
                        ...
                        >
                          {showAllCities ? "Show Less" : `+${lane.cities.length - 3} More`}
                        </button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {(showAllCities ? lane.cities : lane.cities.slice(0, 3)).map((city, index) => (
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400 mb-1">{flightPlan.duration}</div>
                <p className="text-gray-400 text-sm">Flight Duration</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-400 mb-1">{flightPlan.altitude}</div>
                <p className="text-gray-400 text-sm">Cruise Altitude</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-400 mb-1">{flightPlan.batteryUsage}</div>
                <p className="text-gray-400 text-sm">Battery Usage</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400 mb-1">₹{flightPlan.totalCost.toLocaleString()}</div>
              ...
              <div className="flex flex-wrap gap-2">
                {flightPlan.route.map((waypoint, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    {i < flightPlan.route.length - 1 && <span className="text-gray-400">→</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-3">
                <h5 className="text-white font-semibold">Safety & Infrastructure</h5>
                <div className="flex items-center gap-2">
                  <Battery className="w-4 h-4 text-green-400" />
                  <div>
                    <p className="text-gray-400 text-sm">Charging Stations</p>
                    <p className="text-white font-semibold">{flightPlan.chargingStations} available</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-white font-semibold">{flightPlan.emergencyLanding} zones</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Badge className="bg-green-500/20 text-green-400">{flightPlan.safetyRating}</Badge>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <Badge className="bg-green-500/20 text-green-400">{flightPlan.weatherConditions}</Badge>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-white font-semibold">{flightPlan.estimatedSpeed}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-white font-semibold">{flightPlan.fuelEfficiency} km/charge</p>
                </div>
                <div className="text-sm">
                  <p className="text-gray-400">Cost per km</p>
                  <p className="text-cyan-400 font-semibold">₹{Math.round(flightPlan.totalCost / distance)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              <h5 className="text-blue-400 font-semibold mb-2">🤖 AI Flight Recommendations</h5>
              <div className="space-y-2 text-sm">
                <p className="text-gray-300">• Optimal departure time: Early morning for best weather conditions</p>
                <p className="text-gray-300">• Recommended sky lane: {flightPlan.skyLane} for minimal traffic</p>
                <p className="text-gray-300">
                  • Battery optimization: {flightPlan.batteryUsage} usage allows for safe margins
                </p>
                {Number.parseInt(flightPlan.batteryUsage) > 80 && (
                  <p className="text-yellow-300">• Consider charging stop for longer range safety</p>
                )}
              </div>
            </div>
          </CardContent>
      ...
      {(!sourceCity || !destinationCity) && (
        <Card className="bg-black/20 border-blue-500/30">
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Plan Your Aerial Journey</h3>
            <p className="text-gray-400">Select departure and destination cities to generate your flight plan</p>
          </CardContent>
        </Card>
      )}
    </div>
